...
DBackup 采用“GUI 调度 + 归档引擎 + 仓库接口 + 自动化 + 网络服务”的分层结构。设计重点是保持 v1/v2/v3 兼容、失败时保护已有备份，并使手动、定时、实时和网络任务共享稳定接口。
...
`main.cpp` 仅负责初始化 Qt 应用；主窗口使用侧边导航组织概览、备份、恢复、计划、历史、存储与设置。耗时操作在工作线程执行，通过队列调用更新进度和页面，避免跨线程修改控件。
...
新建 v3 归档和增量块使用 AES-256-GCM。恢复会校验相对路径并拒绝通过重解析点逃逸目标根目录；旧 AES-CBC 仅作为 v2 兼容读取路径。
