...
bool readFile(const std::string &path, std::vector<uint8_t> &data) { std::ifstream f(path,std::ios::binary); if(!f)return false; data.assign(std::istreambuf_iterator<char>(f),{}); return !f.bad(); }
bool writeAtomic(const std::string &path,const std::vector<uint8_t>&data){std::string temp=path+".tmp";std::ofstream f(temp,std::ios::binary|std::ios::trunc);if(!f)return false;f.write(reinterpret_cast<const char*>(data.data()),data.size());f.close();if(!f)return false;return MoveFileExW(toWide(temp).c_str(),toWide(path).c_str(),MOVEFILE_REPLACE_EXISTING|MOVEFILE_WRITE_THROUGH)!=0;}
